...
from nanobot.coc.db.models import ModuleChunk, ModuleSource
from nanobot.coc.db.module_content import ModuleImportService
from nanobot.coc.db.module_progress import ModuleProgressService
from nanobot.coc.modules.search import ModuleSearchService
...
class Coc7ModuleTool(Tool):
    """Search imported scenario module facts without loading whole chapters."""

    name = "coc7_module"
    description = (
        "Import channel attachments or local documents as campaign scenario modules, inspect their "
    ...
    def _execute_sync(
        ...
        *,
        ...
        if store.enabled:
            try:
                result["chromadb"] = [
                    store.collection_stats(collection_name("coc7_modules", profile))
                    for profile in configured_profiles()
                ]
            except Exception:
                result["chromadb"] = {"name": "coc7_modules", "error": "unreachable"}
        else:
