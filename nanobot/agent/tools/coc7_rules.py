"""Native CoC7 rule retrieval backed by the campaign rules database."""
...
from nanobot.coc.db.models import RuleChunk, RulePublication, RuleSet
from nanobot.coc.rules.ingest import ensure_bundled_rules_ingested
from nanobot.coc.rules.search import RuleSearchService
from nanobot.ttrpg.rules.embedding import collection_name, configured_profiles
...
class Coc7RulesTool(Tool):
    """Search the indexed CoC 7e SRD without spawning a fresh Python process per query."""

    name = "coc7_rules"
    description = (
        "Search the versioned CoC 7e rules database with exact, full-text, and configured "
        "BGE dense "
    ...
    def _execute_sync(
        ...
        *,
        ...
        if store.enabled:
            try:
                result["chromadb"] = [
                    store.collection_stats(collection_name("coc7_rules", profile))
                    for profile in configured_profiles()
                ]
            except Exception:
                result["chromadb"] = {"name": "coc7_rules", "error": "unreachable"}
        else:
