"""Audit-backed restoration of CoC7 engine state."""
...
class UndoManager:
    ...
    def __init__(self, database: Database, *, max_undo: int | None = None) -> None:
        configured = os.environ.get("COC7_AUDIT_UNDO_LIMIT")
    ...
    def _restore_revision(
        ...
        if isinstance(aggregate, Character):
            ...
            aggregate.occupation = payload.get("occupation", aggregate.occupation)
            aggregate.archetype = payload.get("archetype", aggregate.archetype)
            aggregate.age = str(payload.get("age", aggregate.age))
            aggregate.sex = str(payload.get("sex", aggregate.sex))
