"""Character library — investigators, NPCs, and creatures, decoupled from campaigns."""
...
class CharacterService:
    """Create and manage characters — investigators, NPCs, and creatures — with an
    ...
    def create(
        ...
        *,
        ...
        character_type: str = "investigator",
        ...
        if character_type not in ("investigator", "npc", "creature", "container"):
            raise ValueError(
                "character_type must be 'investigator', 'npc', 'creature', or 'container'"
            )
        sheet = dict(sheet_json or {})
        resolved_occupation = occupation or str(sheet.get("occupation", ""))
        resolved_archetype = archetype or str(sheet.get("archetype", ""))
            ...
            with self.database.transaction() as session:
                ...
                session.add(
                    ToolAudit(
                        ...
                        actor_id=actor_id,
                        tool_name="coc7_character_create",
    ...
    def bind_to_campaign(
        ...
        *,
        ...
        with self.database.transaction() as session:
            ...
            if campaign is None:
                raise CampaignNotFoundError(f"campaign not found: {campaign_id}")
            if character.character_type != "investigator":
                raise CharacterError("only investigators can be bound to campaigns")
    ...
    def update(
        ...
        **fields: Any,
        ...
        allowed = {
            "name", "player_name", "character_type",
            "occupation", "archetype", "age", "sex",
            "residence", "birthplace", "organization",
            "sheet_json",
    ...
    def _info(character: Character) -> CharacterInfo:
        return CharacterInfo(
            ...
            organization=character.organization or "",
