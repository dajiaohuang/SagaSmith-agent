...
SNAPSHOT_FORMAT = "coc7-campaign-snapshot"
...
class CampaignSnapshotService:
    ...
    def capture_from_session(cls, session: Session, campaign_id: str) -> dict[str, Any]:
        ...
        for key, model, fields in _STATE_MODELS:
            ...
            # Only capture investigators in snapshots — NPCs live in the global library.
            if key == "characters":
                conditions.append(model.character_type == "investigator")
            state[key] = [
                _row(item, fields)
    ...
    def restore_in_session(
        ...
        *,
        ...
        # Only restore investigators — NPCs live in the global library and are never deleted on restore.
        session.execute(
            delete(Character).where(
                Character.campaign_id == campaign_id, Character.character_type == "investigator"
            )
        )
