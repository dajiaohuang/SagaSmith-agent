...
| 模式 | play_style | 说明 |
|------|-----------|------|
| **游戏外 (Lobby)** | `lobby` | 简单入口：查看、创建或切换战役，快速建卡后进入 DM |
...
Lobby 不生成长内容，也不启动后台 Subagent；但可同步随机生成一个战役或角色设定，用户确认后再创建。
确认创建单个角色或 NPC 后，会阻塞运行单卡规范化 Subagent；只有规则校验和人物卡模板 XLSX 往返校验全部通过才写入数据库。
