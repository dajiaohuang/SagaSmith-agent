...
- 骰娘模式不替代真实 DM 推进预设剧情，但会持续审计被 @ 的操作和在场角色，并维护可检索记忆。
- 被 @ 要求更新记忆时，默认读取当前消息或被引用消息，并追问是否继续读取前面的群聊记录。
- 被 @ 要求开始战斗时，会先追问参战角色及先攻优势/劣势，再统一投先攻并按 DM 模式主持回合。
- 骰娘模式仅关闭主动剧情推演、环境描写和 NPC 扮演；角色卡、规则、技能、法术、物品、记忆、检定、伤害、治疗和战斗等全部工具能力仍可通过自然语言使用。
- 骰娘模式只输出事实、规则、数据、计算结果、状态变更和必要澄清问题；不会给行动建议，也不会输出任何扮演、氛围、环境描写或 NPC 台词。
- 进入骰娘模式时，当前战役的 NPC 与怪物会临时关联至 DM QQ；无法唯一确定 DM 时会先询问。退出骰娘模式会清除这些临时关联，不影响玩家角色绑定。
- 从 DM 模式切换到骰娘模式不会切换战役；当前战役的进度、场景、背景、公开设定、角色和记忆会继续作为工具问答上下文。
- 开战参战名单、记忆前文确认等追问不会锁死会话。退出战斗、退出骰娘、查看战役、其他命令和普通问题都可以随时打断。
- 例如可直接问“我有什么技能能放”“火球术是什么效果”“我的运动检定加多少”；战役设定编辑仍需返回战役叙事模式。
